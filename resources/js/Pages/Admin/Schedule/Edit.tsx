...
const SchedulingEdit = () => {
    const { schedule, scheduleRoutes, flash } = usePage<PageProps>().props;

    const { data, setData, put, processing, errors, reset } = useForm({
        date: schedule.date,
        time: schedule.time,
        schedule_route_id: schedule.schedule_route_id.toString(),
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.scheduling.update', schedule.id), {
            onSuccess: () => {
                toast.success('Schedule updated successfully.')
            },
            onError: () => {
                toast.error('Failed to update schedule. Please check the form for errors.');
            }
        });
    };
    ...
    return (
        <Layout>
            <Head title={`Edit ● ${schedule.route_name}`} />
            <Title title={`Edit Schedule: ${schedule.route_name}`} />
